...
def explore_embedding_space(
    ...
    for layer in tqdm(range(num_layers)):
        ...
        mean_cos_sim_across_words[f'layer_{layer}'] = round(np.nanmean([1 - cosine(random.choice(word_vectors),
                                                                                   random.choice(word_vectors)) for _ in
                                                                        range(num_samples)]), 3)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(model, 'pcs')):
        os.makedirs(os.path.join(model, 'pcs'))
    ...
    explore_embedding_space(EMBEDDINGS_FULL_PATH, f'{model}/embedding_space_stats.json', 100)
